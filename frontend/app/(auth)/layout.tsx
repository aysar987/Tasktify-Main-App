  ...
  return (
    <main className="grid min-h-dvh bg-white lg:grid-cols-[1.05fr_.95fr]">
      <section className="relative hidden overflow-hidden bg-slate-950 p-12 text-white lg:flex lg:flex-col">
        ...
        <div className="relative mt-auto grid grid-cols-3 gap-4">{[{ icon: UsersRound, value: "1.200+", label: "Penyedia" }, { icon: CheckCircle2, value: "8.500+", label: "Task selesai" }, { icon: ShieldCheck, value: "100%", label: "Terverifikasi" }].map(({ icon: Icon, value, label }) => <div key={label} className="border-l border-slate-700 pl-4"><Icon className="mb-3 size-5 text-orange-400" /><strong className="block text-xl">{value}</strong><span className="text-sm text-slate-400">{label}</span></div>)}</div>
      </section>
