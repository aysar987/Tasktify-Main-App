...
export function AuthForm({ mode }: { mode: "login" | "register" | "reset" | "update" }) {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    {mode === "register" && <><label className="block text-sm font-bold">Nama lengkap<input name="fullName" required autoComplete="name" className={inputClass} /></label><label className="block text-sm font-bold">Username<input name="username" required minLength={3} autoComplete="username" className={inputClass} /></label><label className="block text-sm font-bold">Nomor telepon<input name="phone" type="tel" autoComplete="tel" className={inputClass} /></label></>}
    {mode !== "update" && <label className="block text-sm font-bold">Email<input name="email" type="email" required autoComplete="email" className={inputClass} /></label>}
    {mode !== "reset" && <label className="block text-sm font-bold">{mode === "update" ? "Password baru" : "Password"}<input name="password" type="password" required minLength={8} autoComplete={mode === "login" ? "current-password" : "new-password"} className={inputClass} /></label>}
