    ...
    >
      ...
      <div className="flex flex-col p-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="flex shrink-0 items-center gap-1 text-sm font-bold text-slate-800"><Star className="size-4 fill-amber-400 text-amber-400" />{rating(provider.rating)}</span>
        </div>
        <p className="truncate text-sm text-slate-500">{provider.title}</p>
        <div className="mt-2 flex items-end justify-between gap-3 border-t border-slate-100 pt-2">
          ...
          <span className="flex items-center gap-1 text-xs text-slate-500"><MapPin className="size-3.5" />{provider.location}</span>
        </div>
      </div>
    </Link>
  );
