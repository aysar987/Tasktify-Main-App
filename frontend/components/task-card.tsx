...
export function TaskCard({ task }: { task: Task }) {
  return (
    <Link href={`/tasks/${task.id}`} className="group block rounded-2xl border border-slate-200 bg-white p-5 transition hover:border-orange-300 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><span className="text-xs font-bold uppercase tracking-wider text-slate-400">{task.id} · {task.perspective === "provider" ? "Sebagai provider" : "Sebagai client"}</span><h3 className="mt-1 font-[var(--font-manrope)] text-lg font-extrabold text-slate-950">{task.title}</h3></div><StatusBadge status={task.status} /></div>
